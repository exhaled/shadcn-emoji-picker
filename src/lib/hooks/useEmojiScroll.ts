...
import type { ScrollRefs } from './useSetScrollPanelRef';

export const useEmojiScroll = ({ scrollPanelRef, viewportRef }: ScrollRefs) => {
    ...
    useEffect(() => {
        if (!selectedEmoji || !scrollPanelRef.current || !viewportRef.current) return;
